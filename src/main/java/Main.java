...
public class Main extends EBAnwendung {
	...
	public void bearbeiteTaste(char c) {
		...
		if (isPlayerInFrontOf(Variables.LOCK) && player.getKeys() > 1
			...
			if (isPlayerInFrontOf(Variables.VILLAIN)) {
				...
				player.setMoney(player.getMoney() + ((Money) lvl.getGameField()[Player.getPosX() + x][Player.getPosY() + y]).getValue());
			...
			case 'c':
				switch (m.cmd()) {
					case "glitch":
						...
						break;
					case "swords":
						player.setSwords(Menu.getInstance().cmdSwords());
						break;
					case "keys":
	...
	private boolean isPlayerInFrontOf(String object) {
		switch (object) {
			...
			case Variables.OBSTACLE:
				return lvl.getGameField()[Player.getPosX() + x][Player.getPosY() + y] instanceof Obstacle && !player.isCheat();
	...
	private void isPlayerOnPortal() {
		if (lvl.getGameField()[Player.getPosX()][Player.getPosY()] instanceof Portal) {
			...
			player.go(((Portal) lvl.getGameField()[Player.getPosX()][Player.getPosY()]).getPosX(),
